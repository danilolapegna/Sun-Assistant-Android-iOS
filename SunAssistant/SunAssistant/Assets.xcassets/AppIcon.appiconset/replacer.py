...
def replace_images_in_directory(reference_file, directory=".", recursive=True):
    reference_file_name = os.path.basename(reference_file).lower()
    
    # Scansione della cartella
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.lower().endswith('.png') and file.lower() == reference_file_name:
...
if __name__ == "__main__":
    ...
    reference_file = "logo.png"  # Il file di riferimento deve essere nella directory corrente

    # Usa la directory corrente per impostazione predefinita
    current_directory = os.getcwd()

    # Cerca anche nelle sottocartelle e sostituisci solo se il nome coincide
    replace_images_in_directory(reference_file, current_directory, recursive=True)