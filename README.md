...
## Project Status
This project was a key component of a startup initiative. However, given the fact that the startup project will be subject to a radical change, development up to past point x has been whitelabeled and made open-source (and its github history up to that point has in fact also been entirely deleted). I did it with a clear intent to:
...
I am a pragmatic dev. This doesn't mean that I like to write bad code for the sake of releasing early. Doesn't also mean that I like to neglect things such as performance, modularity, best practices and good code structure. But honestly: I still think that releasing what you have despite imperfect is something you've got to deal with. 
Too often, software engineers fall in love with the idea of perfection—clean, beautifully styled code that ticks every checkbox of "best practices." But the reality is, coding is just a tool we should use to solve problems effectively. 
The goal can't just be to write pretty code for the sake of it; it's to create something functional, valuable, and delivered within timings that are acceptable for the context we are working in. 
Spending hours perfecting the indentation or over-engineering a solution that could be simpler and just as effective isn't just non-pragmatic, it is straight terrible. It just strengthens the wall around the tech people bubble, in addition to wasting time that could be better spent solving real-world problems.

In short: this codebase probably isn't perfect, but it's engineered well enough to represent a first, strong, well modularised iteration to solve the initial problem. Hopefully you'll appreciate it enough for that. 
